...
uk.theta_MLE = fminsearch(@(theta) nloglf_age($$$--fill--$$$),[0.1;0.1]);
lb = [0;0];
ub = [1;1];
china.theta_MLE = fminsearchbnd(@(theta) nloglf_age($$$--fill--$$$),[0.2;0.2],lb,ub);
...
uk.best_val = nloglf_age($$$--fill--$$$,uk);
china.best_val = nloglf_age($$$--fill--$$$,china);
    ...
    uk.nln2 = @(theta) nloglf_age...
        ...
        -(chival_95/2+uk.best_val);
    uk.theta_MLE_ci(1,1) = fzero(uk.nln1,$$$--fill--$$$);
    uk.theta_MLE_ci(1,2) = fzero(uk.nln1,$$$--fill--$$$);
    uk.theta_MLE_ci(2,1) = fzero(uk.nln2,$$$--fill--$$$);
    uk.theta_MLE_ci(2,2) = fzero(uk.nln2,$$$--fill--$$$);
    ...
    china.nln2 = @(theta) nloglf_age...
        ...
        -(chival_95/2+china.best_val);
    china.theta_MLE_ci(1,1) = fzero($$$--fill--$$$);
    china.theta_MLE_ci(1,2) = fzero($$$--fill--$$$);
    china.theta_MLE_ci(2,1) = 0;    % cannot find by fzero
    china.theta_MLE_ci(2,2) = fzero($$$--fill--$$$);
...
%% 6-4 plot the graph : model predection and observed data
    ...
    % uk
    uk.y_theta_MLE = solve_catalytic_age($$$--fill--$$$,$$$--fill--$$$);
    ...
    % china
    china.y_theta_MLE = solve_catalytic_age($$$--fill--$$$,$$$--fill--$$$);
