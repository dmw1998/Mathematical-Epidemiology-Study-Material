...
% 5-1 Fill a blank in "solve_catalytic_MI"
...
% uk
    ...
    % 5-3 compute CI using Chi-square dist.
    uk.theta_MLE_ci = zeros(1,2);
    chi2 = @(x) chi2cdf(x,1) - 0.95;
    chival_95 = fzero(chi2,2);
    uk.best_val = uk.nloglf($$$--fill--$$$);
    
    uk.nln = @(theta) uk.nloglf(theta)-(chival_95/2+uk.best_val);
    uk.theta_MLE_ci(1) = fzero(uk.nln,$$$--fill--$$$);
    uk.theta_MLE_ci(2) = fzero(uk.nln,$$$--fill--$$$);
    
    % 5-4 values and best-fitting plot
...
% china
    ...
    % 5-6 compute CI using Chi-square dist.
    china.theta_MLE_ci = zeros(1,2);
    chi2 = @(x) chi2cdf(x,1) - 0.95;
    chival_95 = fzero(chi2,2);
    china.best_val = china.nloglf($$$--fill--$$$);
    
    china.nln = @(theta) china.nloglf(theta)-(chival_95/2+china.best_val);
    china.theta_MLE_ci(1) = fzero(china.nln,$$$--fill--$$$);
    china.theta_MLE_ci(2) = fzero(china.nln,$$$--fill--$$$);
    
    % 5-7 values and best-fitting plot
    fprintf('Q5. best-fitting value : %f \n',china.theta_MLE);
    fprintf('Q5. CI for force of infection : [%f %f] \n\n',...
