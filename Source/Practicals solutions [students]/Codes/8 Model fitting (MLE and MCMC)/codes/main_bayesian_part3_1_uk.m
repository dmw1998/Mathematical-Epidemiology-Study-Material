...
%% Question 9-1
% 9-1-1 MCMC procedure for various initial 
...
for k = 1 : 10
    ...
    % 9-1-2 Trace and Autocorrelation
    ...
    title('Trace')
    set(gca, 'FontSize', 15)
    saveas(gca, 'Q9-1 trace_for_various_initials','epsc')

%% Question 9-2
% 9-2-1 MCMC procedure for various number of samples
f_mh = @(theta) sum(log(binopdf(Pa,Na,1-exp(-theta.*time_stamp))));
...
for k = 1 : 4
    ...
    saveas(gca, sprintf('Q9-2 trace_autocorre_%s',filename),'epsc')
    ...
    set(gca, 'FontSize', 15)
    filename = sprintf('%3.0e',N(k));
    saveas(gca, sprintf('Q9-3 hist_distribution_%s',filename),'epsc')
    
end
%% Question 10

% 10-1 best fitting values - foi, average age of infection, R0, H
...
fprintf('Q10. best-fitting value : %f \n',theta_MCMC);
fprintf('Q10. average age of infection  : %f \n',A)
fprintf('Q10. basic reproduction number : %f \n',R0)
fprintf('Q10. herd immunity threshold   : %f \n\n',H)

% 10-2 95% Credible interval
temp = sort(samples');
...
fprintf('Q10. CI for force of infection : [%f %f] \n',theta_MCMC_ci(1),theta_MCMC_ci(2));
fprintf('Q10. CI for average age of infection  : [%f %f] \n',A_ci(2),A_ci(1))
fprintf('Q10. CI for basic reproduction number : [%f %f] \n',R0_ci(1),R0_ci(2))
fprintf('Q10. CI for herd immunity threshold   : [%f %f] \n\n',H_ci(1),H_ci(2))

% 10-3 best-fitting plot
