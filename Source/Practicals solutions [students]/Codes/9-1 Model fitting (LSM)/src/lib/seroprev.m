function output = seroprev(ages, foi, diff_age)
%seroprev - calculate seroprevalence for each age with given FOI.
%
% Syntax: output = seroprev(ages, foi, diff_age)
%
% Input: given ages and foi (double), diff_age means the age values whose
%       FOI is differentiated when foi is 1X2 vector.
...
sero(1:check_points-1) = 1-exp(-foi(1)*ages(1:check_points-1));

if (check_points~=length(ages)+1)
    sero(check_points:end) = ...
        1-exp(-diff_age*(foi(1)-foi(2)))*exp(-foi(2)*ages(check_points:end));
end
