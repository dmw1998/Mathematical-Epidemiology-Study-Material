...
% 5-1 MLE procedure
fprintf('start MLE procedure \n')
nloglf_MI = @(theta) -sum(log(max(eps,binopdf(Pa,Na,1-exp(-theta*(time_stamp-0.5))))));
theta_MLE_MI = fminsearch(nloglf_MI,0.2);

% 5-2 compute CI using Chi-square dist.
theta_MLE_MI_ci = zeros(1,2);
chi2 = @(x) chi2cdf(x,1) - 0.95;
chival_95 = fzero(chi2,2); 
best_val_MI = nloglf_MI(theta_MLE_MI);

nln_MI = @(theta) nloglf_MI(theta)-(chival_95/2+best_val_MI);
theta_MLE_MI_ci(1) = fzero(nln_MI,theta_MLE_MI*0.8);
theta_MLE_MI_ci(2) = fzero(nln_MI,theta_MLE_MI*1.2);

% 5-3 best-fitting plot
y_theta_MLE_MI = solve_catalytic_MI(theta_MLE_MI,time_stampc);

figure1 = figure('pos',[10 10 600 400]);
plot(time_stamp,data,'.','MarkerSize',20);  % data plot
hold on; 
plot(time_stampc,y_theta_MLE_MI,'LineWidth',2)    % prediction plot
temp_legend = sprintf('Prediction(\\lambda = %f)',theta_MLE_MI);
legend('Data',temp_legend,'Location','east')
xlabel('Age (yrs)')
ylabel('Proportion positive')
grid on; grid minor;
