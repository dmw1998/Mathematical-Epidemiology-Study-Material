...
nloglf = @(theta) -sum(log(max(eps,poisspdf(data,solve_SEIR(theta,param)))));
theta_MLE = fminsearch(nloglf,2e-5);
...
theta_MLE_ci(1) = fzero(nln,theta_MLE*0.999);
...
temp_legend = sprintf('Prediction(\\beta = %e)',theta_MLE);
...
%% Question 8
% 8-1 R0, H
R0 = theta_MLE*initial(1)/r;
H = 1-1/R0;

R0_ci = theta_MLE_ci*initial(1)./r;
H_ci = 1-1./R0_ci;
